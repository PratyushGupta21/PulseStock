...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("rounded-xl border border-[#1E293B] bg-[#0F172A] text-[#F8FAFC]", className)}
      {...props}
    />
  )
)
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("font-serif text-2xl font-bold leading-none tracking-tight text-[#F8FAFC]", className)}
      {...props}
    />
  )
)

  ...
  return (
    <>
      <Card
        ...
        className="h-full bg-[#0F172A] border border-[#1E293B] p-5 rounded-xl shadow-none flex flex-col justify-between hover:border-[#38BDF8]/50 hover:shadow-lg transition-all cursor-pointer group"
      >
        <CardHeader className="p-0 mb-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              <CardTitle className="text-xl font-bold font-serif text-[#F8FAFC] group-hover:text-[#38BDF8] transition-colors flex items-center gap-1.5">
                {displayTicker}
                <ArrowUpRight className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity text-[#38BDF8]" />
              </CardTitle>
              <p className="text-xs text-[#94A3B8] truncate max-w-[140px]">{displayName}</p>
            </div>
            <Badge
              variant="secondary"
        ...
        <CardContent className="p-0 flex-1 flex flex-col justify-between">
          <div className="mb-2">
            <div className="font-mono text-2xl font-bold text-[#F8FAFC] tracking-tight">
              ${currentPrice > 0 ? currentPrice.toFixed(2) : "..."} <span className="text-xs font-normal text-[#94A3B8]">SUSD</span>
            </div>
          ...
          <Button
            size="sm"
            className="w-full mt-3 font-semibold bg-[#1E293B] text-[#F8FAFC] hover:bg-[#38BDF8] hover:text-[#080C14] border border-[#1E293B] transition-colors"
            onClick={(e) => {
              e.stopPropagation()
              setIsTradeOpen(true)
            }}
